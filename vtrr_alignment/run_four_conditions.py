...
from vtrr_alignment.llm import ChatClient, ChatConfig
from vtrr_alignment.metrics import (
    PackageEvaluation,
...
from vtrr_alignment.retrieval import (
    cached_retriever,
...
DEFAULT_OUTPUT_DIR = ROOT / "results" / "vtrr_alignment"
...
def run_prompt(
    ...
    if condition in ("vtrr", "vtrr_sub", "vtrr_sub_delete"):
        code_dependency = evaluate_code_dependencies(code, package_names)
        # For vtrr_sub / vtrr_sub_delete: find alternatives, substitute (or delete)
        if condition in ("vtrr_sub", "vtrr_sub_delete"):
            ...
            if condition == "vtrr_sub":
                ...
                # vtrr_sub_delete: same repair prompt but NO alternatives
                pre_alt_map = {}
                pre_alt_dict = {}
    ...
    if condition in ("vtrr_sub", "vtrr_sub_delete"):
        ...
        if needs_repair:
            ...
            if condition == "vtrr_sub":
                ...
                # vtrr_sub_delete: no alternatives, empty substitution guide
                alt_map = {}
                alt_context = ""
                sub_alternatives_count = 0
    ...
    # --- Original VTRR repair ---
    elif condition == "vtrr" and (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run Spracks-aligned no-RAG/RAG/VTRR/attack-surface experiments."
    )
    ...
    parser.add_argument(
        "--conditions",
        nargs="+",
        default=["no_rag", "rag_no_attack", "vtrr", "spracks_attack"],
        choices=["no_rag", "rag_no_attack", "vtrr", "vtrr_sub", "vtrr_sub_delete", "spracks_attack"],
    )
